...
export const useAuthStore = defineStore('auth', () => {
   ...
   * 执行人脸登录。
   */
  async function loginWithFace(payload: FaceAuthPayload): Promise<void> {
    const result = await loginByFace(payload);
    setAuth(result.accessToken, result.user);
   ...
   * 保存当前账号的人脸模板。
   */
  async function registerFaceTemplate(payload: FaceAuthPayload): Promise<void> {
    await registerFace(payload);
    ElMessage.success('人脸模板已保存');
  }
